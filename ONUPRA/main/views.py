...
def run_command(file_name, tests_input, tests_output):
    ...
    path = os.path.join(os.path.dirname(__file__),
                        'main', 'media', 'comp_files', '')
    command = ""
    if file_name.endswith(".py"):
        command = "docker run -v " + path + file_name + ":" + file_name + \
            " --rm --read-only python:3.8-alpine python" + file_name
...
def Settings(request):
    ...
    if request.method == 'POST':
        ...
        if "email_edit" in request.POST:
            ...
            print(email, ((not CustomUser.objects.filter(email=email).exists()) and (
                not email.strip() == "") and ('@' in email) and ('.' in email)))
            print(password, (check_password(password, request.user.password)
        ...
        # Если нажали кнопку редактирования пароля
            ...
            if check_password(old_password, request.user.password) and new_password == new_password2:
                ...
                request.user.save()
            print(old_password, new_password, new_password2, check_password(
                old_password, request.user.password), successfully)
            data = {
                'successfully': successfully,
            }
            return JsonResponse(data)
            ...
            print(delete_password, successfully, check_password(
                delete_password, request.user.password))
...
def Competitions(request):
    ...
    #     return redirect('login')
    comp = Competition.objects.filter(actual=True).order_by('-start_time')
    past_comp = Competition.objects.filter(
        actual=False).order_by('-start_time')
    content = {
        'comp': comp,
        'past_comp': past_comp,
    }
...
def Competition_now(request, id, taskid):
    ...
    if not request.user.is_authenticated:
        ...
        data = {
            'id': atmpt.id,
            'link_competition': atmpt.link_competition.title,
            'link_task': atmpt.link_task.title,
